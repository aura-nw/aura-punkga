...
const MainButton: React.FC<MainButtonProps> = ({
    ...
    return (
        <button
            ...
            onClick={onClick}
        >
            {leadingIcon && children && <span className="leading-icon"><Image color={'#414141'} className={iconStyleClasses} width={size === 'large' ? 24 : 18} height={size === 'small' ? 24 : 18} src={leadingIcon} alt='' /></span>}
            {iconOnly && !children && <span className="icon-only"><Image className={iconStyleClasses} width={size === 'large' ? 24 : 18} height={size === 'small' ? 24 : 18} src={iconOnly} alt='' /></span>}
            {children}
            {trailingIcon && children && <span className="trailing-icon"><Image className={iconStyleClasses} width={size === 'large' ? 24 : 18} height={size === 'small' ? 24 : 18} src={trailingIcon} alt='' /></span>}
        </button>
    );
};
