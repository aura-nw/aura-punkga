    ...
    <Listbox
      ...
      {({ open }) => (
        <>
          <Listbox.Label className='block text-sm font-medium leading-6 text-gray-900'>{label}</Listbox.Label>
          <div className='relative'>
            <Listbox.Button className='relative w-full cursor-default rounded-[8px] bg-black py-[1px] md:py-[7px] pl-[10px] pr-[57px] text-left text-white focus:outline-none text-sm leading-6 lg:h-10 border border-white font-normal'>
              <span className={`flex items-center text-[14px] leading-[24px] ${className}`}>
                {selected?.key ? (
                  <span className='block truncate font-normal'>{selected.value}</span>
                ) : (
                  <span className='block truncate text-medium-gray font-normal'>{placeholder}</span>
                )}
              </span>
            ...
            <Transition
              ...
              enterFrom='opacity-0'
              enterTo='opacity-100'>
              <Listbox.Options className='absolute z-10 mt-1 max-h-56 w-full min-w-fit whitespace-nowrap rounded-[12px] overflow-auto bg-black text-base shadow-lg ring-1 ring-black focus:outline-none sm:text-sm'>
                {options.map((option) => (
                  <Listbox.Option
                    key={option.key}
                    className={({ active }) =>
                      classNames(
                        option.key == selected?.key ? 'bg-brand-default text-black' : 'text-white',
                        'relative cursor-default select-none py-2 pl-3 pr-16'
                      )
                    }
                    value={option}>
