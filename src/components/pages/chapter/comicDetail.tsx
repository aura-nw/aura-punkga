...
export default function ComicDetail({
  ...
  return (
    <div
      className={`${expandDetail ? ' z-10' : 'z-[1] delay-500 transition-all'
    ...
    >
      ...
      ></div>
      <div
        className={`relative flex-auto  h-full duration-500 transition-all ${expandDetail
          ? 'pk-width__ml bg-white'
          : 'w-1/4 overflow-auto bg-light-gray'
          }`}
      >
        <div
          className={`${expandDetail ? 'z-10 opacity-100' : ' opacity-0 -z-10'
            }`}
        >
          <div
        ...
        >
          <div className={` duration-500 transition-all flex gap-5`}>
            ...
            />
            <div
              className={`flex-1 flex flex-col ${expandDetail ? 'gap-[5px]' : 'gap-[10px]'
                }`}
            >
              <div
                className={`font-bold ${expandDetail ? 'text-black' : 'text-second-color'
              ...
              >
                ...
                {expandDetail && (
                  <StatusLabel status={data.status?.type}>
                    {t(data.status?.text)}
                  </StatusLabel>
                )}
              </div>
              <p
                className={`${expandDetail
                  ? 'opacity-100 max-h-[100px]'
                  : 'opacity-0 max-h-0 mt-[-10px]'
                  } font-semibold text-xl leading-6 duration-500 transition-all text-second-color overflow-auto`}
              >
                {data.authors.map((author, index) => (
                  <Fragment key={index}>
                    <span className="text-second-color font-[600] first:hidden">
                      ,{' '}
                    </span>
          ...
          <div className="flex">
            {data.releaseDate && !moment().diff(moment(data.releaseDate)) && (
              <div
                className={` text-subtle-dark transition-all whitespace-nowrap overflow-hidden ${expandDetail
                  ? 'max-w-[300px] min-w-[260px] max-h-[50px]'
                  : 'max-w-[0px] min-w-[0px] max-h-0'
                  } `}
              >
                {t('Release date')}:{' '}
          ...
          {data.chapters.find(
            (chapter) => chapter.status === CHAPTER_STATUS.UPCOMING,
          )?.date && (
              <div
                className={`flex gap-2 items-center text-second-color text-sm font-semibold italic ${expandDetail ? ' h-6 opacity-100' : 'opacity-0 h-0'
