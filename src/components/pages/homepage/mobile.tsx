...
export default function MobileVersion() {
  ...
  const { postList, setPostData, selectedTags, setSelectedTags } =
  ...
  return (
    <div className="bg-background py-4 text-white min-h-screen">
        ...
        <div className="">
          ...
          >
            {events.map((event, index) => (
              <SwiperSlide
                key={index}
                className="!h-[97px] !w-[143px] rounded-md overflow-hidden"
              >
                {event?.image && (
      ...
      <div
        className={`sticky z-10 py-5 transition-all bg-background ${
          scrollDir == "up" ? "top-14" : "-top-5"
        }`}
      >
        <Swiper
          slidesPerView="auto"
      ...
      <InfiniteScroll
        ...
        loader={
          <h4 className="w-full text-center font-medium text-sm">Loading...</h4>
        }
      >
