...
function CollectionDetail() {
  ...
  const { data } = useSWR({ key: 'fetch_launchpad', id: slug }, ({ id }) => getStoryLaunchPadDetail(id as string), {
    refreshInterval: 60000,
  })

  const { data: minted, refetch } = useReadContract({
    abi: abi,
...
console.log(minted)
  ...
  const allowance = useReadContract({
    ...
    args: [address, launchpadContractAddress],
  })
  const allowanceBalance = (allowance.data as any) || 0
      ...
      <div className='py-8 px-4 pk-container lg:flex lg:gap-8 lg:bg-white lg:rounded-md lg:p-4 lg:mt-8'>
        ...
        <div className='mt-8 lg:mt-0 lg:w-full'>
          <div className='flex gap-2.5'>
            {getChip()} {data.max_supply <= nftMinted && <Chip color='error'>{t('Sold out')}</Chip>}
          </div>
          <div className='mt-4'>
            ...
            <div className='lg:flex lg:gap-4 lg:items-center'>
              ...
              <div className='mt-4 text-sm font-medium'>
                {+data.max_mint_per_address == 0
                  ? t('Unlimited')
                  : locale == 'en'
                  ? `Max ${+data.max_mint_per_address} per wallet`
                  : `Tối đa ${+data.max_mint_per_address} NFT mỗi ví`}
              </div>
            </div>
            <div className='my-8 grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-4'>
              <div className='py-2 px-3 border border-border-secondary rounded-md bg-neutral-50'>
                ...
                <div className='mt-1.5 font-semibold'>{`${data.max_supply} NFTs`}</div>
              </div>
              {!isUpcoming && (
                <div className='py-2 px-3 border border-border-secondary rounded-md bg-neutral-50'>
                  <div className='text-text-teriary text-sm font-medium'>{t('Collected')}</div>
                  <div className='mt-1.5 font-semibold'>{`${nftMinted} NFTs`}</div>
                </div>
              )}
              <div className='py-2 px-3 border border-border-secondary rounded-md bg-neutral-50'>
                <div className='text-text-teriary text-sm font-medium'>{t('Value')}</div>
                <div className='mt-1.5 font-semibold'>{`${
                  BigInt(data.mint_price) / BigInt(10) ** BigInt(18)
                } USDT`}</div>
              </div>
            </div>
            {isLive && (
                ...
                <div className='mt-4 flex gap-4'>
                  ...
                  <div className='flex min-h-10 font-semibold'>
                    ...
                    <div
                      ...
                      onClick={() =>
                        setAmount((amount) =>
                          amount < (+data.max_mint_per_address || data.max_supply - nftMinted) ? amount + 1 : amount
                        )
                      }>
                      +
                    </div>
                ...
                <div className='mt-4'>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (
                        balance > +((BigInt(data.mint_price) / BigInt(10) ** BigInt(18)) * BigInt(amount)).toString()
                      ) {
                        setConfirmOpen(true)
                      } else {
            ...
            <div className='w-full border-t border-border-secondary mt-10 pt-6'>
              <div className='flex gap-4 [&>div]:text-sm [&>div]:font-semibold [&>div]:text-text-teriary [&>div]:px-3 py-0.5 [&>div]:rounded-md [&>div.active]:text-text-brand-defaul [&>div.active]:bg-brand-100 [&>div]:cursor-pointer'>
              ...
              <div className='text-sm space-y-3 mt-3'>
                {tab == 1 ? (
                  <>
                    <div>
                      Launchpad contract: <span className='text-info-default'>{shorten(launchpadContractAddress)}</span>
                    </div>
                    <div>
                      Creator: <span className='text-info-default'>{shorten(data?.creator_address)}</span>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div>
                      Licensor ID: <span className='text-info-default'>{data?.ip_asset_id}</span>
                    </div>
                  </>
                )}
              </div>
        ...
        <div className='bg-white py-8 px-4 max-w-[343px] w-[90vw] md:max-w-[547px]'>
          <div className='rounded-mlg p-3 flex gap-1 bg-feedback-error-50'>
            ...
            <div className='text-text-error-primary-3 text-sm'>
              {t('You don’t have enough USDT to collect this NFT')}
            </div>
          </div>
          <div className='mt-8 text-sm font-medium text-text-teriary'>{t('Please send USDT to your address')}</div>
          <div
