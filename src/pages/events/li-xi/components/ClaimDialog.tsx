...
export default function ClaimDialogs({ cb }: { cb?: () => void }) {
  ...
  const [address, setAddress] = React.useState(account?.activeWalletAddress || '')
      ...
      <BootstrapDialog
        ...
        <DialogTitle sx={{ m: 0, p: 2 }} id='customized-dialog-title' className='flex flex-col items-center'>
          <div className='text-center text-[#9F1515] text-2xl font-black-han-sans'>
            {status === 'SUCCEEDED' ? t('All prizes claimed!') : t('Claim all prizes')}
          </div>
        </DialogTitle>
        <IconButton
          aria-label='close'
          ...
          {!loading && !requestId ? (
            <div className='flex flex-col items-center gap-4 mb-4 w-[300px]'>
              <div className='text-center'>
                Please make sure your wallet address is correct. This progess is irreversible
              </div>
              <TextField onChange={setAddress} value={address} size='md' placeholder={t('Enter your wallet address')} />
            </div>
          ) : (
            <>
              {status === 'CREATED' && (
