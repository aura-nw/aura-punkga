...
const BulletPoint = ({ className }: { className?: string }) => (
  <div className={`${className} w-[10px] h-[10px]  rotate-45 rounded-sm bg-[#F0C865]`}></div>
)
...
export default function Lixi() {
  ...
  const {
  ...
  } = useSWR(
    {
      key: 'fetch-user-lixi',
      id: account?.id,
      selectedLixiType: selectedLixi?.type,
    },
    async ({ selectedLixiType }) => {
      try {
        const { data } = await eventService.liXi.getUserLixi()
        if (!data || data.errors) return null
        let newSelectedLixi
        switch (selectedLixiType) {
          case 'yellow':
            if (data.yellow_lixi_list?.[0]) {
              newSelectedLixi = {
                type: 'yellow',
        ...
        if (!newSelectedLixi) {
          newSelectedLixi = data.yellow_lixi_list?.[0]
            ? { type: 'yellow', id: data.yellow_lixi_list[0].id }
            : data.red_lixi_list?.[0]
            ? { type: 'red', id: data.red_lixi_list[0].id }
            : data.blue_lixi_list?.[0]
            ? { type: 'blue', id: data.blue_lixi_list[0].id }
            : undefined
        }
        setSelectedLixi(newSelectedLixi)
    ...
    <div
      ...
      <div
        ...
        <svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
          <path d='M15 17L10 12L15 7' stroke='#fff' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round' />
        </svg>
        <div className='text-white text-lg font-medium leading-relaxed'>Event List</div>
      </div>
      <div className='first-section relative h-full flex flex-col justify-center items-center'>
        ...
        <div className='relative  mt-[300px] md:mt-0'>
          <Image src={podium} className='relative z-10 w-[calc(100vw-40px)] md:w-[384px]' alt='' />
          <div className='absolute top-[90px] left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 w-[150px]'>
            {selectedLixi && userLixi?.[selectedLixi.type]?.aggregate.count ? (
              <EventButton onClick={() => {}} className='w-[150px]'>
                <RewardDialogs selectedLixi={selectedLixi} rewardCallback={() => mutateUserLixi()} />
              </EventButton>
            ) : (
              <div className='text-[#FABA77] text-center'>
                {t('You don’t have any Li Xi, refer your friends to earn more.')}
              </div>
            )}
          </div>
        </div>
        ...
        <div className='flex flex-col gap-4 w-full'>
          <div className='relative px-6 py-4 rounded-b-[6px] border border-[#D52121] bg-gradient-to-b from-[rgba(117,20,20,0.50)] via-[rgba(133,7,7,0.50)] to-[rgba(244,63,63,0.50)] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] backdrop-blur-[6.45px]'>
            <Image
              ...
              alt='den_long'
            />
            <Image src={cane} className='absolute lg:hidden -top-2 -left-4 max-w-[calc(100vw-10px)]' alt='cane' />
            ...
            <div className='text-white'>{t('Complete missions to earn 3 types of LiXi')}</div>
            <div className='flex justify-start  gap-4 items-center text-white text-sm font-medium'>
              <BulletPoint /> {t('Red: Submit a character of Literature Infinity event.')}
            </div>
            <div className='flex justify-start gap-4 items-center text-white text-sm font-medium'>
              <BulletPoint /> {t('Gold: Collect a fortune number & sign in Punkgame account.')}
            </div>
            <div className='flex justify-start  gap-4 items-center text-white text-sm font-medium'>
          ...
          <div className='grid grid-cols-12 justify-between items-start gap-4 mb-32 md:mb-0'>
            <div className='relative px-6 py-4 flex flex-col gap-4 col-span-12 rounded-[6px] border-[0.5px] border-[#FFD66B] bg-gradient-to-b from-[#FDF5CB] via-[#FDF5CB] to-[#FFF9DB] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)]'>
              <div
                ...
                style={{
                  background: 'linear-gradient(98deg, #FA7519 -2.01%, #FFCB7E 109.77%)',
                }}
              />
              <div
              ...
              />
              <div className='text-[#8E0B09] font-normal font-black-han-sans text-2xl'>{t('Fortune Number')}</div>
              <div className='flex justify-between items-center'>
                <div
              ...
              <div className='flex flex-col justify-start items-center'>
                {fortuneNumbers?.map((ele, index) => (
                  <div className='flex justify-between items-center w-full gap-4'>
                    ...
                    <div className={!ele.used ? '' : 'text-[#6D6D6D]'}>{ele.code}</div>
                    {ele.used ? (
                      <Image src={ic_check} className='w-4 h-4' alt='ic_check' />
                    ) : (
                      <Copy2Clipboard text={ele.code}>
                        <Image src={ic_copy} className='w-4 h-4 cursor-pointer' alt='ic_copy' />
                      </Copy2Clipboard>
                    )}
                  </div>
                ))}
              ...
              <div className='text-[#F4741B] mt-4'>
                {t('Both referrer and referree receives a Blue LiXi when a code being used')}
              </div>
            </div>{' '}
