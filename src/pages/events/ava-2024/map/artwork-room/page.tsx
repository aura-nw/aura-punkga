...
import MangaFrame from 'components/pages/event/ava-2024/assets/artwork-frame.png'
...
const Content = ({ selected }) => {
  ...
  const mangaName =
    (locale == 'en'
      ? selected.artwork.manga_languages.find((l) => l.language_id == 1)?.title
      : selected.artwork.manga_languages.find((l) => l.language_id == 2)?.title) ||
    selected.artwork.manga_languages.find((l) => l.is_main_language)?.title
  return (
      ...
      <div className='px-5 -mt-[58px] z-50 relative py-6 bg-[linear-gradient(0deg,#191919_68.29%,rgba(25,25,25,0.00)_100%)]'>
        <div className='space-y-1'>
          ...
          <div className='flex w-full items-center justify-between'>
            ...
            <div className='flex items-center text-sm gap-1.5'>
              {selected?.artwork?.manga_total_likes?.likes || 0}
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='24'
                height='24'
        ...
        <div className='mt-4 w-full'>
          <Button
            ...
            onClick={() =>
              isMobile
                ? window.open(`/comic/${selected.artwork.slug}`, '_blank')
                : window.open(`/comic/${selected.artwork.slug}/chapter/1`, '_blank')
            }>
            {t('Read artwork')}
          </Button>
        </div>
