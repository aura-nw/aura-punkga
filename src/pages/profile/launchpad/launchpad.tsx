...
function Launchpad() {
  ...
  useEffect(() => {
    const newData =
      listLaunchpads?.data?.filter((item) => {
        if (status.key === 'All status') {
          return item.name.toLowerCase().includes(searchValue.toLowerCase())
        } else {
          return (
            item.name.toLowerCase().includes(searchValue.toLowerCase()) && LaunchpadStatus[item.status] === status.value
          )
        }
      }) ?? []
            ...
            <div className='flex justify-between'>
              <div className='flex gap-3'>
                <TextField
                  ...
                  leadingComponent={<Image src={SearchIcon} className='w-[22px] h-[22px]' alt='' />}
                />
                <FilledSelect
                  label=''
                  className='text-[#333333] h-full rounded-md min-w-[250px]'
                  selected={status}
                  onChange={setStatus}
              ...
              <Button size='sm' color='dark' onClick={() => router.push('/profile/launchpad/create')}>
                {t('New Launchpad')}
              </Button>
            </div>
                  ...
                  <div className='flex flex-col gap-3 mt-3'>
                    {listLaunchpadData?.map((launchpad: LaunchpadType, idx: number) => {
                      return (
                        <div
                          key={idx}
                          className='flex items-center px-[18px] py-[11px] text-subtle-dark text-sm leading-[18px] bg-white rounded-[10px]'>
                          <div className='w-[56px] mr-14'>{idx + 1}</div>
                          <div className='w-[200px] flex-1'>
                            <Tooltip title={launchpad.name} placement='top-start'>
                              <Link
                                href={`/profile/launchpad/${launchpad.id}`}
                                className='pr-1 overflow-hidden text-ellipsis whitespace-nowrap'>
                                {launchpad.name}
                              </Link>
                            </Tooltip>
                          </div>
                          ...
                          <div className='w-[200px] flex-1'>
                            {moment(launchpad.start_date).format('HH:mm yyyy-MM-DD')}
                          </div>
                          <div className='w-[200px] flex-1'>
                            {moment(launchpad.end_date).format('HH:mm yyyy-MM-DD')}
                          </div>
                          <div className='flex-1 text-end'>
                            {moment(launchpad.updated_at).format('HH:mm yyyy-MM-DD')}
                          </div>
                        </div>
                      )
                    })}
                  ...
                  <div className='flex justify-center p-4'>
                    <div className='flex flex-col gap-10'>
                      ...
                      <div className='flex flex-col gap-2 items-center'>
                        <span className='text-[#414141] text-[20px] font-bold leading-[25px]'>
                          Have not lauchpad now
                        </span>
                      </div>
                    </div>
                  </div>
