...
const withApi = (Component: React.FC<any>) => (props: any) => {
  ...
  const updateLaunchpadDraft = async (id: string, launchpad: any) => {
    const { data } = await privateAxios.put(`${config.REST_API_URL}/launchpad/${id}/edit-draft`, launchpad)
    return data
  }

  const updateLaunchpadUnpublish = async (id: string, launchpad: any) => {
    const { data } = await privateAxios.put(`${config.REST_API_URL}/launchpad/${id}/edit-unpublish`, launchpad)
    return data
  }
