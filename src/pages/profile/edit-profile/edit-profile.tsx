...
export default function EditProfile({ updateProfile }) {
  ...
  return (
    <div className="bg-background pt-5 px-5 lg:px-0 lg:pt-10 flex justify-center">
      <div className="md:pk-container">
        <div className="w-[624px] p-8 bg-background rounded-[10px] relative">
          <div className="text-white font-medium text-xl leading-7">{t("Edit profile")}</div>
          <div className="flex flex-col justify-center items-center gap-[14px] mt-8">
            ...
            <div className="relative w-[146px] shrink-0 h-[146px] rounded-[19.5px] border-[2.43px] border-black">
              <Image src={preview || account?.image || NoImg.src} alt="" width={146} height={146} className="w-full h-full rounded-[19.5px] border-[2.43px] border-[#454545]" />
              <div className="absolute -top-2 -right-2 cursor-pointer shadow-md z-10 w-8 h-8 bg-white hover:bg-slate-100 rounded-full p-2">
                <input ref={profilePicture} onChange={onSelectFile} type="file" className="bg-black absolute inset-0 opacity-0" id="change-profile-picture-input" />
            ...
            <div className="w-full flex flex-col gap-4 mt-8">
              <div className="">
                <div className="font-medium text-sm mb-2 leading-5 text-white">{t("Username")}</div>
                <TextField
                  value={username}
                  onChange={setUsername}
                  placeholder={t("Enter your username")}
                  className="text-sm text-[#ABABAB] font-normal md:font-bold leading-6 !py-[5px] !px-[10px] w-full md:aspect-[84/12] border-[#ABABAB]"
                />
              </div>
              <div className="">
                <div className="font-medium text-sm mb-2 leading-5 text-white">{t("Gender")}</div>
                <FormControl fullWidth>
                  <StyledSelect
                    value={gender ? gender.key : ""}
                    onChange={(e) => setGender({ key: e.target.value as any, value: t(e.target.value as any) })}
                    displayEmpty
                    className="text-sm text-[#ABABAB] font-normal md:font-bold leading-6 w-full md:aspect-[84/12]"
                  >
              ...
              <div className="">
                <div className="font-medium text-sm mb-2 leading-5 text-white">{t("Date of birth")}</div>
                <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={locale === "vn" ? vi : undefined}>
                  <StyledDatePicker
                    value={birthdate}
                    format="dd/MM/yyyy"
                    onChange={(newValue) => setBirthdate(newValue)}
                    className="text-sm border-whie text-[#ABABAB] font-normal md:font-bold leading-6 md:w-full md:aspect-[84/12] w-[330px] max-h-10"
                  />
              ...
              <div className="">
                <div className="flex justify-between items-center">
                  <div className="font-medium text-sm mb-2 leading-5 text-white">{t("Short bio")}</div>
                ...
                <AutoGrowingTextField
                  ...
                  className="text-sm text-[#ABABAB] font-normal md:font-bold leading-6 !py-[5px] !px-[10px] w-full md:aspect-[84/12] min-h-[82px] border-[#ABABAB]"
                />
              </div>
              <div className="">
                <div className="font-medium text-sm mb-2 leading-5 text-white">{t("Email")}</div>
                <TextField
                  value={account?.email}
                  disabled
